...
import {entryTypeChipClass, formatRelative} from '@/utils/format';
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
                  <Typography.Text style={{ fontSize: 'var(--font-size-sm)' }}>{item.details}</Typography.Text>
                    <Tag className={entryTypeChipClass(item.entryType)}
                         style={{fontSize: 'var(--font-size-xxs)', margin: 0}}>{item.entryType}</Tag>
                </div>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                  <Typography.Text type="secondary" style={{ fontSize: 'var(--font-size-xs)' }}>{item.entryTitle}</Typography.Text>
                    {item.src &&
                        <Tag className="lynks-chip" style={{fontSize: 'var(--font-size-xxs)', margin: 0}}>{item.src}</Tag>}
